...
class Student_st:
    ...
    def update_data(self):
        var_ph=self.phone.get()
        Pattern = re.compile("[6-9][0-9]{9}")
        if self.dep.get()!="" or self.batch.get()!="" or self.name.get()!="" or self.gender.get()!="" or self.dob.get()!="":
            messagebox.showerror("Error", "Not Accessable for Students", parent=self.root)
            ...
            try:
                ...
                if Update>0:
                    conn=mysql.connector.connect(host="localhost", username="root", password="Subhadip@321#", database="face_recognition")
                    my_cursor=conn.cursor()
                    my_cursor.execute("update student set sem=%s,phone=%s,place=%s where roll=%s",(
                                                                                                self.sem.get(),
                                                                                                self.phone.get(),
                                                                                                self.place.get(),
                                                                                                self.roll.get()
    ...
    def reset_data(self):
        ...
        messagebox.showinfo("Reset", "Reset successful",parent=self.root)
